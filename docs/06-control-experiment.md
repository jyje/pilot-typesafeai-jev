...
| --- | --- | --- | --- |
| Screen | every model and setting | 31 x 3 (NIM 31 x 1) | which ones work, and how fast |
...
A configuration goes on to the main stage when at least 90% of its calls returned a usable answer and
its median latency is 60 s or less. **The rule uses availability, never accuracy**, because choosing
finalists by accuracy on the same messages would flatter them. Messages, not repeats, limit how far a
result generalizes, so every interval is a 95% bootstrap over messages.
...
a result and is never retried.

## Results

Main stage, first 5 runs of every configuration, 60 messages. Accuracy is the share of runs on the
expected route. The full tables are in the [notebook](../src/notebooks/03-case04-structured-control.ipynb)
and in `src/experiments/data/tables/`.

| Configuration | Accuracy | 95% interval | Consistency | Median latency |
| --- | --- | --- | --- | --- |
| **Jev** | 0.967 | 0.917 to 1.000 | 1.000 | 0.6 s |
| gpt-5.6-luna, low (best of ChatGPT) | 0.967 | 0.920 to 1.000 | 0.993 | 2.3 s |
| gpt-5.6-sol, medium | 0.940 | 0.873 to 0.990 | 0.993 | 2.9 s |
| gpt-6-astra, medium (lowest of ChatGPT) | 0.900 | 0.823 to 0.967 | 0.987 | 3.1 s |
| glm-5.3-flash | 0.907 | 0.837 to 0.970 | 0.983 | 19.9 s |
| nemotron-3-ultra, thinking on | 0.899 | 0.840 to 0.952 | 0.939 | 11.9 s |
| nemotron-3-nano-omni, thinking on | 0.858 | 0.775 to 0.930 | 0.965 | 9.9 s |

![Route accuracy of every configuration](images/control-accuracy.png)

- **Accuracy: no difference detected.** No configuration is clearly better or worse than Jev, and the
  intervals of all fifteen overlap Jev's. The best ChatGPT configuration ties it at 0.967. Overlap is not
  proof of equality: with 60 messages the intervals are about 0.1 wide.
- **Reasoning effort did not help in a consistent way.** Within a ChatGPT model, low, medium, and high
  differ by at most 0.04 and not in one direction (`gpt-5.6-luna`: 0.967, 0.953, 0.963).
...
- **Injection: nothing got through, and one control refused too much.** At most 2% of runs on the
...
- **Availability differs a lot.** In the screen every ChatGPT setting was usable at 99% or better. On NIM,
  ...
  thinking off returned `503` or `400` often, and `glm-5.3` and `kimi-k3` timed out at 600 s on all five
  core messages. The three NIM configurations that passed still lost between under 1% and 12% of calls to
  schema, parsing, or provider failures.

## What this does and does not show

- It shows that on these messages a chat model with structured output routes as accurately as Jev, so
  **accuracy is not what sets Jev apart here**. What differs is speed, the same answer every time, and
  that Jev returns a probability instead of a label.
...
- To keep the run short, NIM was screened with one run per message instead of three, and `glm-5.3` and
  `kimi-k3` on the five core messages only. Both are documented deviations, and the rule did not change.
