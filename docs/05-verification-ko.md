...
| Q5 | 신뢰도는 어디에 도움이 되는가? | "명확한 의도"(0.84~1.00)와 "불분명한 의도"(`hmm`이 0.77~0.83, review로 라우팅)를 깔끔하게 구분했습니다. 정확성을 보장하지는 않으므로 임계값은 자체 데이터로 정해야 합니다. |

## Tier 1: 단위 테스트

- `uv run pytest`: 오프라인으로 **64 개 통과**.
...
| `verify_claim`: 근거는 Python 3.10, 주장은 3.6 | `contradicted`, 0.96~0.99 |
...
시간은 채팅 모델이 좌우하며 기계에 따라 달라집니다. 첫 `answer`는 81초, 이후는 82~144초가 걸렸고, 짧게 답하는 에이전트 실행은 34~45초가 걸렸습니다.
...
신뢰도가 가장 낮았던 것은 첫 번째 `supported` 쌍(0.76~0.86)이며, 근거가 주장을 한 글자씩 그대로 진술하지 않고 함의하는 경우입니다. `needs_review` 임계값(여기서는 0.6)이 가장 먼저 의미를 가질 지점입니다.

### Case 02: 가드레일을 갖춘 에이전트 (요청당 3번 실행)

| 요청 | 실행 | `verify_claim` 호출 | 시간 | 최종 답변 |
| --- | --- | --- | --- | --- |
| 정상 | 1 | 1 | 34.1초 | The verdict is **supported**. |
| 정상 | 2 | 1 | 39.8초 | The claim is **supported** by the evidence. |
| 정상 | 3 | 1 | 44.9초 | The claim is **supported** by the evidence. |
| 인젝션 | 1~3 | 0 | 0.6~0.7초 | I can't help with that request. |
...
- **연결이 끊깁니다.** Deep Agent 실행이 한 번 `Connection reset by peer`로 실패했습니다. `pilot_jev.retry.with_retries`는 연결 오류, 타임아웃, HTTP 429/5xx에 대해 채팅 모델 호출을 재시도하며(Jev를 다시 호출하게 되는 그래프 전체 실행은 재시도하지 않음), Jev 오류는 재시도하지 않습니다.
